module UserAuth
  module TokenCommons
    ...
    def get_user_id_from(payload)
      # with_indifferent_accessでシンボルでも文字列でもハッシュの値にアクセスできるようにする
      # payloadのキーがシンボルか文字列かはJWTのライブラリに依存するために使用
      # 内部的にはpayloadのハッシュキーを文字列に変換している
    ...
    # user_idを用いてDBに保存するための`JWT ID`を生成するためのメソッド
